...
/*
 * The reading half of a sweep: a per-point table of what happened, and an
 * effect table ranking each axis by eta-squared, the share of a metric's
 * variance lying between the axis's levels rather than within them (0 to 1).
 * It ranks; it does not test. A row with fewer than `THIN_SEEDS` a level is
 * `thin`, and one under `RESOLVED_ETA2` is `unresolved` with the seed count
 * the spread would need. See `docs/experiments.md` §1.D and §5.
 *
 * Every metric folds its timeline to one number and carries a default fold
 * (`peak` for transients, `window` for cumulative counters, `last` for
 * standing measures); any fold can be asked for by name: `net_fst@slope`.
 * `mean` and `slope` skip the warm-up.
 */
...
/** Simulated seconds `mean` and `slope` ignore at the start of a run (the opening latch storm). */
...
/**
 * Everything `analyze` can rank. Columns are the `sample` table's; `json`
 * paths reach into the stored `Sample`. `docs/experiments.md` §3 is the catalogue.
 */
export const METRICS: Record<string, MetricSpec> = {
  ...
  // Cumulative since the run began; `@last` for the total.
  ...
  // Net motion: snapshots, meaningless without a control run.
...
/** Column heading for a key: the label, plus the fold when not the default; an unknown key is its own label. */
...
export interface TrialRow {
  ...
  /** The run's whole `Params`; absent on a row built by hand. */
...
/** Every finished run in the library, folded the same way, whatever sweep it belonged to. */
...
interface RunRow {
  ...
  sweep: string | null;
  /*
   * Present only from `libraryTrials`. Setup, not parameters; a regression
   * blind to them credits whichever parameter moved alongside.
   */
  seconds?: number;
  soup_count?: number;
  field_cells?: number;
  ...
  for (const r of runs) {
    ...
    for (const a of asked) {
      ...
      if ('column' in src) {
        ...
        // A ratio of two windows, not the window of a ratio.
        if (a.summary === 'window') {
        ...
        // What one unit of signal reads as on the way into `x`, at this run's sense scale.
...
/** Eta-squared per axis per metric, plus which way the metric moves. Nulls are dropped, not zeroed. */
export function effects(trials: TrialRow[], metrics?: string[]): Effect[] {
  ...
  for (const axis of axes) {
    for (const metric of keys) {
      ...
      // Relative, not `sd > 0`: a constant metric still carries an ulp of
      // spread, and eta-squared over that is a ratio of two rounding errors.
...
/** Eta-squared at or above which a row is read as resolved. */
