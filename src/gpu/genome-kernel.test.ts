...
import {
  ...
  PLASTIC_LEN, X_OUT, SENSE_SCALE, STATE_DIMS, TASTE, T_OUT, W_IN, W_NET, W_SELF,
} from '../chem-layout.ts';
...
describe('the genome shader matches the genome layout', () => {
  it('carries the same offsets chem-layout.ts derives', () => {
    ...
    const want: Record<string, number> = {
      ...
      B_STATE, F_OUT, F_BASE, L_OUT, L_BASE,
  ...
  it('writes the sixteen floats a body the host unpacks', () => {
    // h(4) + emit(4) + taste(4) + four heads. The host reads them back by this
    // stride, so a mismatch shifts every field by a body.
    expect(shaderConst('OUT_STRIDE')).toBe(4 + 4 + 4 + 4);
  });
  ...
  it('takes its uniform in the order the host writes it', () => {
    ...
    expect(fields).toEqual([
      ...
      'sCruise', 'sTurn', 'sAlign', 'sSep', 'energyCh',
